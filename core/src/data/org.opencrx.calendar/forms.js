...
function showTodoForm(todo, mod, repeatOne, confirmRepeat)
{
	...
	if(todo!=null)
		...
		{
			if( cals[i].uid!=undefined && ((todo!=null && todo.res_id==cals[i].uid) || (!cals[i].permissions_read_only && (globalVisibleCalDAVTODOCollections.indexOf(cals[i].uid)!=-1 || calSelected==cals[i].uid))))
			{
...
function showEventForm(date, allDay, calEvent, jsEvent, mod, repeatOne, confirmRepeat)
{
	...
	var calSelected = $('.resourceCalDAV_item.resourceCalDAV_item_selected').attr('data-id');
		for(var i=0;i<cals.length;i++)
		{
			if( cals[i].uid!=undefined && ((calEvent!=null && calEvent.res_id==cals[i].uid) || (!cals[i].permissions_read_only && (globalVisibleCalDAVCollections.indexOf(cals[i].uid)!=-1 || calSelected==cals[i].uid))))
			{
