...
function netSaveSettings(inputResource, inputSettings, isFormSave)
{
	...
	$.ajax({
		...
		processData: true,
		data: '<?xml version="1.0" encoding="utf-8"?><D:propertyupdate xmlns:D="DAV:"><D:set><D:prop><I:settings xmlns:I="http://inf-it.com/ns/dav/">'+inputSettings+'</I:settings></D:prop></D:set></D:propertyupdate>',
		dataType: 'xml',
		complete: function(xml, textStatus)
		{
			switch(textStatus)
			{
				case 'success' :
					if(isFormSave && isAvaible('Settings'))
						showSettingsForm($('.resourceSettings.resourceSettings_selected').attr('data-type'));
				case 'nocontent':
					return false;
					break;
			}
		...
		if((isAvaible('CalDavZAP') && !isCalDAVLoaded) || (isAvaible('CardDavMATE') && !isCardDAVLoaded))
		{
			if(isAvaible('CalDavZAP'))
			{
				if(!globalDefaultCalendarCollectionActiveAll)
					...
					{
						var uidPart=globalSettings.activecalendarcollections[i].match(RegExp('^(https?://)(.*)', 'i'))[1];
						var uidPart2=globalSettings.activecalendarcollections[i].match(RegExp('^(https?://)(.*)', 'i'))[2];
				...
				if(!globalDefaultTodoCalendarCollectionActiveAll)
					...
					{
						var uidPart=globalSettings.activetodocollections[i].match(RegExp('^(https?://)(.*)', 'i'))[1];
						var uidPart2=globalSettings.activetodocollections[i].match(RegExp('^(https?://)(.*)', 'i'))[2];
						var uidPart3=globalAccountSettings[0].userAuth.userName;
						var uid = uidPart+uidPart3+'@'+uidPart2;
	...
	$.ajax({
		...
		complete: function(xml, textStatus)
		{
			...
			$(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response').each(function(index, element){
				$(element).children().filterNsNode('propstat').each(function(pindex, pelement){
					...
					if(typeof inputResource!='undefined' && typeof inputResource.collectionTypes!='undefined' && inputResource.collectionTypes!=null && inputResource.collectionTypes.indexOf('calendar')!=-1 ||
						...
						if(isAvaible('CalDavZAP') && resources.children().filterNsNode('resourcetype').children().filterNsNode('calendar').length==1 && resources.children().filterNsNode('resourcetype').children().filterNsNode('collection').length==1)
						{
							...
							if(typeList.indexOf('vevent')!=-1)
							{
								...
								globalResourceCalDAVList.insertResource({typeList:typeList, listType:'vevent', ecolor: color, timestamp: resultTimestamp, uid: uidBase+href, timeOut: inputResource.timeOut, displayvalue: displayvalue, userAuth: inputResource.userAuth, resourceIndex: indexR, url: baseHref, accountUID: origUID, href: href, hrefLabel: inputResource.hrefLabel, showHeader: inputResource.showHeader, permissions: {full: permissions, read_only: read_only}, crossDomain: inputResource.crossDomain, withCredentials: inputResource.withCredentials, interval: null, waitInterval: null, displayEventsArray: new Array(), pastUnloaded: '', fcSource: null, subscription: false, urlArray:null, ignoreAlarms:ignoreAlarms,webdav_bind:webdav_bind, syncRequired:syncRequired, checkContentType: checkContentType, syncToken: synctoken, someChanged:someChanged}, indexR, true);
							...
							if(typeList.indexOf('vtodo')!=-1)
							{
								...
								globalResourceCalDAVList.insertResource({typeList:typeList, hrefArray: new Array(), listType:'vtodo', ecolor: color, timestamp: resultTimestamp, uid: uidBase+href, timeOut: inputResource.timeOut, displayvalue: displayvalue, userAuth: inputResource.userAuth, resourceIndex: indexR, url: baseHref, accountUID: origUID, href: href, hrefLabel: inputResource.hrefLabel, showHeader: inputResource.showHeader, permissions: {full: permissions, read_only: read_only}, crossDomain: inputResource.crossDomain, withCredentials: inputResource.withCredentials, interval: null, waitInterval: null, displayEventsArray: new Array(), pastUnloaded: '', fcSource: null, subscription: false, urlArray:null, ignoreAlarms:ignoreAlarms,webdav_bind:webdav_bind,syncRequired:syncRequired, checkContentType: checkContentType, syncToken: synctoken, someChanged:someChanged}, indexR, false);
								if(globalAccountSettings[indexR]!=undefined)
						...
						if(typeof inputResource!='undefined' && typeof inputResource.collectionTypes!='undefined' && inputResource.collectionTypes!=null && inputResource.collectionTypes.indexOf('addressbook')!=-1 || typeof inputResource=='undefined' || inputResource.collectionTypes==null)
							if(isAvaible('CardDavMATE') && resources.children().filterNsNode('resourcetype').children().filterNsNode('addressbook').length==1 && resources.children().filterNsNode('resourcetype').children().filterNsNode('collection').length==1)
							{
			...
			if(typeof globalResourceCalDAVList!='undefined' && globalResourceCalDAVList!=null)
				globalResourceCalDAVList.removeOldResources(uidBase, resultTimestamp);
			if(typeof globalResourceCardDAVList!='undefined' && globalResourceCardDAVList!=null)
				globalResourceCardDAVList.removeOldResources(uidBase, resultTimestamp);
...
function CalDAVnetLoadCollection(inputCollection, forceLoad, allSyncMode, recursiveIterator, collections)
{
	...
	{
		if(inputCollection.uid==undefined || inputCollection.subscription || (!inputCollection.syncRequired && !forceLoad && allSyncMode) || (!inputCollection.someChanged && !globalCalDAVInitLoad &&allSyncMode) || ((globalSettings.eventstartpastlimit==null && globalSettings.eventstartfuturelimit==null))&&!inputCollection.syncRequired)
		{
			...
			if(inputCollection.uid!=undefined)
			{
				if(!inputCollection.syncRequired && globalCalDAVInitLoad)
				{
					...
					if((globalSettings.eventstartpastlimit==null && globalSettings.eventstartfuturelimit == null)||globalLimitLoading=='')
					{
					...
					else if(globalOnlyCalendarNumberCount==globalOnlyCalendarNumber || globalOnlyCalendarNumberCount==globalTodoCalendarNumber)
						updateMainLoader();
				}
	...
	else // if inputCollection.forceSyncPROPFIND is undefined or false
	{
		...
		else
		{
			...
			else if(inputCollection.listType=='vtodo')
			{
				...
				var pastInterval = '', futureInterval = '';
				if(globalSettings.eventstartpastlimit!=null && (allSyncMode || globalLimitLoading=='pastTodo'))
				{
					if(allSyncMode)
				...
				if(globalSettings.eventstartfuturelimit!=null && (allSyncMode || globalLimitLoading=='futureTodo'))
				{
					if(allSyncMode)
				...
				if(!allSyncMode)
					requestText='<?xml version="1.0" encoding="UTF-8"?><L:calendar-query xmlns:L="urn:ietf:params:xml:ns:caldav"><D:prop xmlns:D="DAV:"><D:getcontenttype/><D:getetag/><L:calendar-data/></D:prop><L:filter><L:comp-filter name="VCALENDAR"><L:comp-filter name="VTODO"><L:time-range'+(globalLimitLoading=='pastTodo' ? pastInterval : futureInterval)+'/></L:comp-filter></L:comp-filter></L:filter></L:calendar-query>';
				else
			...
			$(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode(new RegExp('^(sync-)?response$')).each(
				function(index, element)
				{
					...
					if(globalSettings.eventstartpastlimit!=null || globalSettings.eventstartfuturelimit!=null)
					{
						...
						{
							var uid=inputCollection.uid+hrefVal.replace(RegExp('.*/', ''), '');
...
//				inputCollection.syncToken=$(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('sync-token').text();
			...
			if((globalSettings.eventstartpastlimit!=null || globalSettings.eventstartfuturelimit!=null) && !isXMLEmpty)
			{
				if(collections.length>0)
				{
					...
					if(globalOnlyCalendarNumberCount==globalOnlyCalendarNumber || globalOnlyCalendarNumberCount==globalTodoCalendarNumber)
						updateMainLoader();
				}
				return false;
			}
			if((globalSettings.eventstartpastlimit!=null || globalSettings.eventstartfuturelimit!=null) && forceLoad && !allSyncMode)
...
//			$('#ResourceCalDAVTODOList [data-id="'+inputCollection.uid+'"]').addClass('r_operate');
		...
		if(evCount==0)
		{
			if(inputCollection.listType=='vevent')
				...
				$('#ResourceCalDAVTODOList [data-id="'+inputCollection.uid+'"]').removeClass('r_operate');
			if(allSyncMode && globalLimitLoading=='')
			{
				if(inputCollection.typeList.indexOf('vevent')!=-1)
						globalAccountSettings[inputCollection.resourceIndex].calendarNo--;
				if(inputCollection.typeList.indexOf('vtodo')!=-1)
