TYPE=VIEW
query=select `ip`.`OBJECT_ID` AS `item_position`,`b`.`OBJECT_ID` AS `simple_booking` from (((`crx`.`oocke1_depotreportitem` `ip` join `crx`.`oocke1_depotreport` `r` on((`ip`.`P$$PARENT` = `r`.`OBJECT_ID`))) join `crx`.`oocke1_bookingperiod` `bp` on((`r`.`BOOKING_PERIOD` = `bp`.`OBJECT_ID`))) join `crx`.`oocke1_simplebooking` `b` on(((`b`.`POSITION` = `ip`.`POSITION`) and (`b`.`VALUE_DATE` >= `bp`.`PERIOD_STARTS_AT`) and ((`b`.`VALUE_DATE` < `bp`.`PERIOD_ENDS_AT_EXCLUSIVE`) or isnull(`bp`.`PERIOD_ENDS_AT_EXCLUSIVE`)) and ((`b`.`BOOKING_STATUS` >= `r`.`BOOKING_STATUS_THRESHOLD`) or (`r`.`BOOKING_STATUS_THRESHOLD` = 0) or isnull(`r`.`BOOKING_STATUS_THRESHOLD`)))))
md5=9e0b8ffcd5476f0d1ef7120f380858b4
updatable=1
algorithm=0
definer_user=root
