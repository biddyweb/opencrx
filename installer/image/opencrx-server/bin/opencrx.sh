...
if [ "$1" = "START" ] ; then
  ...
  if [ -e @@INSTALLDIR@@/data/crx_2_5/startdb.sh ] ; then 
    @@INSTALLDIR@@/data/crx_2_5/startdb.sh START &
    sleep 3
  fi
  if [ -e @@INSTALLDIR@@/bin/tomcat.sh ] ; then 
    export JAVA_OPTS="$JAVA_OPTS -Dopencrx.CRX.jdbc.driverName=org.hsqldb.jdbcDriver"
    export JAVA_OPTS="$JAVA_OPTS -Dopencrx.CRX.jdbc.url=jdbc:hsqldb:hsql://127.0.0.1:@@HSQLDB_PORT@@/CRX_2_5"
...
if [ "$1" = "STOP" ] ; then
  ...
  if [ -e @@INSTALLDIR@@/data/crx_2_5/startdb.sh ] ; then 
    @@INSTALLDIR@@/data/crx_2_5/startdb.sh STOP
  fi
